...
def preprocess_graph(adj, layer, norm='sym', renorm=True):
    ...
    # rowsum[rowsum == 0] = 1
    # if norm == 'sym':
    #     degree_mat_inv_sqrt = sp.diags(np.power(np.maximum(rowsum, 1e-5), -0.5).flatten())
    ...
    # return adjs
    device = adj.device
    ident = torch.eye(adj.shape[0], device=device)

    if renorm:
        adj_ = adj + ident.to_sparse()
    else:
        adj_ = adj
    ...
    if norm == 'sym':
        degree_mat_inv_sqrt = torch.diag(torch.pow(torch.maximum(rowsum, torch.tensor(1e-5, device=device)), -0.5))
        adj_normalized = degree_mat_inv_sqrt @ adj_.to_dense() @ degree_mat_inv_sqrt
        laplacian = ident - adj_normalized.to_sparse()
    elif norm == 'left':
        degree_mat_inv = torch.diag(torch.pow(rowsum, -1.))
        adj_normalized = degree_mat_inv @ adj_.to_dense()
        laplacian = ident - adj_normalized.to_sparse()

    reg = [1] * layer
    adjs = []
    for i in range(len(reg)):
        adjs.append(ident - (reg[i] * laplacian.to_dense()))
